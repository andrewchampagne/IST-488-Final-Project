...
If providing any code, write it in R and ALWAYS wrap it in triple-backtick fenced blocks with the `r` language tag (like ```r ... ```) so it renders with syntax highlighting. Explain the code step-by-step. Always use the provided context to answer, and do not rely on any information outside of it.
